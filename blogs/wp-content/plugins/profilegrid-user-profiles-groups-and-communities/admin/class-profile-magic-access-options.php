...
//put your code here
	...
	public function profile_magic_save_access_meta($post_id)
	{
            ...
            {
                if( sanitize_text_field( $_POST['pg_meta_box_nonce'] ) || wp_verify_nonce( sanitize_text_field( $_POST['pg_meta_box_nonce'] ), 'save_post_access_meta' ) ){
		if(isset($post_id))
		{
			if(isset($_POST['pm_enable_custom_access']))
			{
				update_post_meta($post_id,'pm_enable_custom_access', sanitize_text_field($_POST['pm_enable_custom_access']));	
			}
			else
			{
			...
			{
				update_post_meta($post_id,'pm_content_access',sanitize_text_field($_POST['pm_content_access']));
			}
			
			if(isset($_POST['pm_content_access_group']))
			{
				update_post_meta($post_id,'pm_content_access_group',sanitize_text_field($_POST['pm_content_access_group']));
			}
		}
            }
            }
				  ...
				  if ( is_user_logged_in() ) 
				  {
					...
					if(get_post_meta($id,'pm_content_access_group',true)!='all')
					{
                                                ...
                                                if(!in_array(get_post_meta($id,'pm_content_access_group',true),$user_group))
						{
                                                    ...
                                                    if(isset($groupinfo)){$group_name = $groupinfo->group_name;}else{$group_name = '';}
							 $err = __('Only members of <a href="%s" target="_blank">%s</a> group can view this page.','profilegrid-user-profiles-groups-and-communities');
                            ...
                            if ( is_user_logged_in() ) 
				  {
                                        ...
                                        if($uid!=$author_id)
                                        {
                                            if(!in_array($uid,$author_friends))
                                            {
                                                ...
                                                $err = __('Only those members who are friends of <a href="%s" target="_blank">%s</a> can view this page.','profilegrid-user-profiles-groups-and-communities');
					...
					$user_group = get_user_meta($uid,'pm_group',true);
                                        ...
                                        if(empty($is_group_member))
                                        {
                                            ...
                                            foreach($author_groups as $agid)
                                            {
                                                ...
                                                $group_page_link = add_query_arg( 'gid',$agid,$group_page_url );
                                                if(isset($groupinfo)){$group_name = $groupinfo->group_name;}else{$group_name = '';}
