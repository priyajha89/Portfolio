...
class Profile_Magic_Admin {
	...
	public function pg_activation_redirect() {
		...
		if ( $dbhandler->get_global_option_value( 'pg_activation_popup','0' ) == '1' ) {
	...
	public function enqueue_scripts() {
		...
		if ( is_user_logged_in() ) :
			...
			wp_register_script( 'pg-extensions-js', plugin_dir_url( __FILE__ ) . 'js/pg-extensions-js.js', array( 'jquery' ), $this->version, true );
                        wp_localize_script( $this->profile_magic, 'pm_ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ), 'nonce' => wp_create_nonce('ajax-nonce') ) );
			$error                              = array();
			...
			if ( ! isset( $upload_requirements['pg_cover_photo_minimum_width'] ) || empty( $upload_requirements['pg_cover_photo_minimum_width'] ) ) {
					...
					$upload_requirements['error_min_cover_width']      = sprintf( __( 'Image dimensions are too small. Minimum size is %1$d by 300 pixels.', 'profilegrid-user-profiles-groups-and-communities' ), $upload_requirements['pg_cover_photo_minimum_width'] );
	...
	public function profile_magic_admin_menu() {
		...
		add_submenu_page( 'pm_manage_groups', __( 'Profile Fields', 'profilegrid-user-profiles-groups-and-communities' ), __( 'Forms', 'profilegrid-user-profiles-groups-and-communities' ), 'manage_options', 'pm_profile_fields', array( $this, 'pm_profile_fields' ) );
...
<div class="pg-no-offer"><?php esc_html_e( 'Sorry, no offers available right now.', 'profilegrid-user-profiles-groups-and-communities' ); ?></div>
	...
	public function pm_fields_list_for_email() {
		...
		echo '<optgroup label="' . esc_attr__( 'Comman Fields', 'profilegrid-user-profiles-groups-and-communities' ) . '" >';
			...
			foreach ( $groups as $group ) {
				...
				if ( isset( $fields ) ) :
					foreach ( $fields as $field ) {
						echo '<option value="{{' . esc_attr( $field->field_key ) . '}}">' . esc_html__( $field->field_name ). '</option>';
					}
				endif;
				...
				<tbody>
						<tr>
							...
							<td class="pm-field-input">              
								<input id="pm_user_avatar" type="hidden" name="pm_user_avatar" class="icon_id" value="
								<?php
						...
						<tr>
							...
							<td class="pm-field-input">              
								<input id="pm_cover_image" type="hidden" name="pm_cover_image" class="cover_icon_id" value="
								<?php
	...
	public function profile_magic_update_user_fields( $user_id ) {
            ...
            if ( isset($post['pg_user_profile_nonce'] ) && wp_verify_nonce( $post['pg_user_profile_nonce'], 'pg_user_profile' ) ) {
			  $dbhandler      = new PM_DBhandler();
				$pmrequests   = new PM_request();
	...
	public function pm_save_custom_permalink_option() {
		if ( ! is_admin() ) {
			return;
                ...
                $pm_sanitizer = new PM_sanitizer;
                $post = $pm_sanitizer->sanitize($_POST);
		if ( isset( $post['permalink_structure'], $post['pg-permalinks-nonce'], $post['pm_blog_base'] ) && wp_verify_nonce(  $post['pg-permalinks-nonce'] , 'pg-permalinks' ) ) {
			$dbhandler = new PM_DBhandler();
