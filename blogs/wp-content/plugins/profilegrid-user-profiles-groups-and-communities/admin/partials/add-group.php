...
if(filter_input(INPUT_POST,'submit_group'))
{
	...
	if (!wp_verify_nonce($retrieved_nonce, 'save_pm_add_group' ) ) die( esc_html__('Failed security check','profilegrid-user-profiles-groups-and-communities') );
	...
	do_action('pg_groupleader_assign_remove',$gid,$is_leader,$group_leaders,$post['is_group_leader'],$post['group_leaders']);
        wp_safe_redirect( esc_url_raw('admin.php?page=pm_manage_groups') );exit;
}
  ...
  <form name="pm_add_group" id="pm_add_group" method="post">
    ...
    <div class="content">
      ...
      <div class="childfieldsrow" id="grouplimit_html" style=" <?php if(!empty($row) && $row->is_group_limit==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
        ...
        <?php wp_dropdown_pages($group_page_args); 
        ...
        <div class="uiminput pm_select_required">
          <select name="associate_role" id="associate_role">
            ...
            <?php
			  ...
			  {?>
            <option value="<?php echo esc_attr($key);?>" <?php if(!empty($row) && $row->associate_role==$key) echo 'selected'; ?>><?php echo esc_html($role['name']); ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
      ...
      <div class="childfieldsrow" id="groupleaderhtml" style=" <?php  if(!empty($row) && $row->is_group_leader==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
        <div class="uimrow">
            ...
            <div class="uiminput">
                <input type="text" name="group_options[admin_label]" id="group_options[admin_label]" value="<?php if(!empty($group_options) && isset($group_options['admin_label'])) echo esc_attr($group_options['admin_label']);?>">
              <div class="errortext"></div>
            </div>
          ...
          <div class="uiminput <?php if(!empty($row) && $row->is_group_leader==1){echo 'pm_group_leader_name pm_select_required';}?>">
                <select name="group_leaders[primary]" id="group_leaders">
                  ...
                  <option value="<?php echo esc_attr($user->ID);?>" <?php if (!empty($group_leaders) && isset($group_leaders['primary']) && $group_leaders['primary']==$user->ID){echo 'selected';} ?>><?php echo esc_html($user->user_login);?></option>
                  <?php endforeach;?>
                </select>
              <div class="errortext"></div>
      ...
      <div class="uimrow">
        ...
        <div class="uiminput">
          <input type="url" name="group_options[redirect_url]" id="group_options[redirect_url]" value="<?php if(!empty($group_options) && isset($group_options['redirect_url'])) echo esc_url($group_options['redirect_url']);?>">
          <div class="errortext"></div>
        </div>
      ...
      <div class="childfieldsrow" id="notification_html" style=" <?php if(!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1){echo 'display:block;';} elseif($id==0){echo 'display:block;';}else{ echo 'display:none;';} ?>">
       ...
       <div class="uimrow">
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo '';}?>" >
          <select name="group_options[on_registration]" id="on_registration">
            ...
            <?php
			  ...
			  {?>
                          <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){if(isset($group_options['on_registration']) && $group_options['on_registration']==$tmpl->id){ echo 'selected';}}elseif($tmpl->tmpl_name === 'On Joining Group'){echo 'selected';}  ?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
      ...
      <div class="uimrow pg-close-group-related-field" style="<?php if($id==0 || (!empty($group_options) && isset($group_options['group_type']) && $group_options['group_type']=='open')){ echo "display:none";}?>">
          ...
          <div class="uiminput <?php if(!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1 && isset($group_options['group_type']) && $group_options['group_type']=='closed'){echo '';}?>" >
            <select name="group_options[on_request_denied]" id="on_request_denied">
              <option value=""><?php esc_html_e('Select Email Template','profilegrid-user-profiles-groups-and-communities');?></option>
              <?php
                            foreach($email_template as $tmpl)
                            {?>
              <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_request_denied']) && $group_options['on_request_denied']==$tmpl->id){ echo 'selected';}}elseif($tmpl->tmpl_name === 'On Request Denial'){echo 'selected';}  ?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
              <?php }
                            ?>
            </select>
            <div class="errortext"></div>
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo '';}?>">
          <select name="group_options[on_user_activate]" id="on_user_activate">
            ...
            <?php
			  foreach($email_template as $tmpl)
			  {?>
            <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_user_activate']) && $group_options['on_user_activate']==$tmpl->id){ echo 'selected';}}elseif($tmpl->tmpl_name === 'User Account Activated'){echo 'selected';} ?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo '';}?>">
          <select name="group_options[on_user_deactivate]" id="on_user_deactivate">
            ...
            <?php
			  foreach($email_template as $tmpl)
			  {?>
            <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_user_deactivate']) && $group_options['on_user_deactivate']==$tmpl->id){ echo 'selected';}} elseif($tmpl->tmpl_name === 'User Account Suspended'){echo 'selected';}?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo '';}?>">
          <select name="group_options[on_password_change]" id="on_password_change">
            ...
            <?php
                          foreach($email_template as $tmpl)
                          {?>
            <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_password_change']) && $group_options['on_password_change']==$tmpl->id){ echo 'selected';}}elseif($tmpl->tmpl_name === 'Password Successfully Changed'){echo 'selected';} ?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
                          ?>
          </select>
          <div class="errortext"></div>
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo '';}?>">
          <select name="group_options[on_account_deleted]" id="on_account_deleted">
            ...
            <?php
                          foreach($email_template as $tmpl)
                          {?>
                          <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_account_deleted']) && $group_options['on_account_deleted']==$tmpl->id){ echo 'selected';}}elseif($tmpl->tmpl_name === 'User Account Deleted'){echo 'selected';} ?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
                          ?>
          </select>
          <div class="errortext"></div>
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo '';}?>">
          <select name="group_options[on_membership_terminate]" id="on_membership_terminate">
            ...
            <?php
                          foreach($email_template as $tmpl)
                          {?>
            <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_membership_terminate']) && $group_options['on_membership_terminate']==$tmpl->id){ echo 'selected';}}elseif($tmpl->tmpl_name === 'Membership Terminated'){echo 'selected';} ?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
                          ?>
          </select>
          <div class="errortext"></div>
     ...
     <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo '';}?>">
       <select name="group_options[on_published_new_post]" id="on_published_new_post">
         ...
         <?php
                       foreach($email_template as $tmpl)
                       {?>
         <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_published_new_post']) && $group_options['on_published_new_post']==$tmpl->id){ echo 'selected';}}elseif($tmpl->tmpl_name === 'New User Blog Post'){echo 'selected';} ?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
         <?php }
                       ?>
       </select>
       <div class="errortext"></div>
     </div>
     <div class="uimnote"><?php esc_html_e('Send this message to the users when their blog post is apporved and published.','profilegrid-user-profiles-groups-and-communities');?><a target="_blank" href="https://profilegrid.co/documentation/new-group-or-edit-group/"><?php esc_html_e('More','profilegrid-user-profiles-groups-and-communities');?></a></div>
    </div>
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_notification']) && $group_options['enable_notification']==1)){echo '';}?>">
          <select name="group_options[on_admin_reset_password]" id="on_admin_reset_password">
            ...
            <?php
			  foreach($email_template as $tmpl)
			  {?>
                          <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_admin_reset_password']) && $group_options['on_admin_reset_password']==$tmpl->id) { echo 'selected';}} elseif($tmpl->tmpl_name === 'Password Reset by Group Manager'){echo 'selected';}?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
      ...
      <div class="childfieldsrow" id="admin_notification_html" style=" <?php if(!empty($group_options) && isset($group_options['enable_group_admin_notification']) && $group_options['enable_group_admin_notification']==1){echo 'display:block;';} elseif($id==0){echo 'display:block;';}else{ echo 'display:none;';} ?>">
        ...
        <div class="uiminput <?php if(!empty($group_options) && isset($group_options['enable_group_admin_notification']) && $group_options['enable_group_admin_notification']==1 && $group_options['group_type']=='closed' ){echo '';}?>">
          <select name="group_options[on_membership_request]" id="on_membership_request">
            <option value=""><?php esc_html_e('Select Email Template','profilegrid-user-profiles-groups-and-communities');?></option>
            <?php
			  foreach($email_template as $tmpl)
			  {?>
                          <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_membership_request']) && $group_options['on_membership_request']==$tmpl->id){ echo 'selected';}}elseif($tmpl->tmpl_name === 'Membership Request'){echo 'selected';} ?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
        ...
        <div class="uiminput <?php if($id==0 || (!empty($group_options) && isset($group_options['enable_group_admin_notification']) && $group_options['enable_group_admin_notification']==1)){echo '';}?>">
          <select name="group_options[on_admin_assignment]" id="on_admin_assignment">
            <option value=""><?php esc_html_e('Select Email Template','profilegrid-user-profiles-groups-and-communities');?></option>
            <?php
			  foreach($email_template as $tmpl)
			  {?>
            <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_admin_assignment']) && $group_options['on_admin_assignment']==$tmpl->id) { echo 'selected'; }} elseif($tmpl->tmpl_name === 'Group Manager assignment'){echo 'selected';}?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
        ...
        <div class="uiminput <?php if(!empty($group_options) && isset($group_options['enable_group_admin_notification']) && $group_options['enable_group_admin_notification']==1){echo '';}?>">
          <select name="group_options[on_admin_removal]" id="on_admin_removal">
            <option value=""><?php esc_html_e('Select Email Template','profilegrid-user-profiles-groups-and-communities');?></option>
            <?php
			  foreach($email_template as $tmpl)
			  {?>
            <option value="<?php echo esc_attr($tmpl->id);?>" <?php if(!empty($group_options)){ if(isset($group_options['on_admin_removal']) && $group_options['on_admin_removal']==$tmpl->id){ echo 'selected'; }} elseif($tmpl->tmpl_name === 'Group Manager Removal'){echo 'selected';}?>><?php echo esc_html($tmpl->tmpl_name); ?></option>
            <?php }
			  ?>
          </select>
          <div class="errortext"></div>
