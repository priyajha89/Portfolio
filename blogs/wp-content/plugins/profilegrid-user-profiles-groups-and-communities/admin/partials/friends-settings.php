...
if(filter_input(INPUT_POST,'submit_settings'))
{
	...
	if (!wp_verify_nonce($retrieved_nonce, 'save_friends_settings' ) ) die(esc_html__('Failed security check','profilegrid-user-profiles-groups-and-communities') );
	...
	{
		if(!isset($post['pm_friends_panel'])) $post['pm_friends_panel'] = 0;
                if(!isset($post['pm_sending_email_on_friend_request'])) $post['pm_sending_email_on_friend_request'] =0;
  ...
  <form name="pm_user_settings" id="pm_user_settings" method="post">
    ...
    <div class="content">
      ...
      <div class="uimrow">
       ...
       <div class="childfieldsrow" id="requestemailhtml" style=" <?php if($dbhandler->get_global_option_value('pm_sending_email_on_friend_request','0')==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
