...
if(filter_input(INPUT_POST,'submit_settings'))
{
	...
	if (!wp_verify_nonce($retrieved_nonce, 'save_performance_settings' ) ) die(esc_html__('Failed security check','profilegrid-user-profiles-groups-and-communities') );
	$exclude = array("_wpnonce","_wp_http_referer","submit_settings");
	$post = $pmrequests->sanitize_request($_POST,$identifier,$exclude);
        if(!isset($post['pm_combine_js'])) $post['pm_combine_js'] = 0;
         if(!isset($post['pm_combine_css'])) $post['pm_combine_css'] = 0;
        //print_r($post);
	if($post!=false)
	{
