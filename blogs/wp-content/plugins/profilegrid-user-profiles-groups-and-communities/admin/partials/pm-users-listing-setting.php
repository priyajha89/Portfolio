...
if(filter_input(INPUT_POST,'submit_settings'))
{
	...
	if (!wp_verify_nonce($retrieved_nonce, 'save_users_lisitng_settings' ) ) die(esc_html__('Failed security check','profilegrid-user-profiles-groups-and-communities') );
	$exclude = array("_wpnonce","_wp_http_referer","submit_settings","pm_field_list");
	$post = $pmrequests->sanitize_request($_POST,$identifier,$exclude);
	if($post!=false)
	{
		if(!isset($post['pm_show_search_bar'])) $post['pm_show_search_bar'] = 0;
                if(!isset($post['pm_show_users_count'])) $post['pm_show_users_count'] =0;
		if(!isset($post['pm_show_advance_search_button'])) $post['pm_show_advance_search_button'] = 0;
		if(!isset($post['pm_show_search_reset_button'])) $post['pm_show_search_reset_button'] = 0;
		if(!isset($post['pm_show_search_sortby'])) $post['pm_show_search_sortby'] = 0;
...
<div class="uimagic">
  <form name="pm_users_lsiting_settings" id="pm_user_settings" method="post">
    ...
    <div class="content">
      ...
      <div class="childfieldsrow" id="searchhtml" style=" <?php if($dbhandler->get_global_option_value('pm_show_search_bar','1')==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
      ...
      <div class="uimrow">
       ...
       <div class="childfieldsrow" id="sortbyhtml" style=" <?php if($dbhandler->get_global_option_value('pm_show_search_sortby','1')==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
