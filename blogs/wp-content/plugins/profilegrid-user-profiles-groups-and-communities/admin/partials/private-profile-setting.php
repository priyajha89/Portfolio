...
if(filter_input(INPUT_POST,'submit_settings'))
{
	...
	if (!wp_verify_nonce($retrieved_nonce, 'save_private_profile_settings' ) ) die( esc_html__('Failed security check','profilegrid-user-profiles-groups-and-communities') );
...
<div class="uimagic">
    ...
    <div class="content">
      ...
      <div class="childfieldsrow" id="enable_private_profile_html" style="<?php if($dbhandler->get_global_option_value('pm_enable_private_profile',0)==1){echo esc_attr('display:block;');} else { echo esc_attr('display:none;');} ?>">
        ...
        <div class="uimnote"><?php esc_html_e("Decide wether you wish to continue displaying user profile cards on Group pages or not. If you chose to display, the cards will no longer link to their respective user profile pages.",'profilegrid-user-profiles-groups-and-communities');?></div>
      </div>
