...
<div class="childfieldsrow" id="paidgrouphtml" style=" <?php if(!empty($group_options) && isset($group_options['is_paid_group']) && $group_options['is_paid_group']==1){echo 'display:block;';} else { echo 'display:none;';} ?>">
  <div class="uimrow">
    ...
    <div class="uiminput <?php if(!empty($group_options) && isset($group_options['is_paid_group']) && $group_options['is_paid_group']==1){echo 'pm_required';}?>">
        <input type="number" name="group_options[group_price]" id="group_price" value="<?php if(!empty($group_options) && isset($group_options['group_price']))echo esc_attr($group_options['group_price']);?>" />
...
<div class="uimrow">
  ...
  <div class="uimnote"><?php echo sprintf(__("Select a user registration form for this group. ProfileGrid Default Group Form option will automatically create a form using profile fields in this group. Other form options are fetched from RegistrationMagic. For more information <a href='%s' target='_blank'>CLICK HERE</a>","profilegrid-user-profiles-groups-and-communities"),esc_url($pg_url));?><a target="_blank" href="https://profilegrid.co/documentation/new-group-or-edit-group/"><?php esc_html_e('More','profilegrid-user-profiles-groups-and-communities');?></a></div>
</div>
