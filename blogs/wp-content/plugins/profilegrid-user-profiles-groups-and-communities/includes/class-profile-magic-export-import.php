...
class PM_Export_Import{
  ...
  public function pm_import_users_from_csv($post)
  {
    ...
    echo '<strong>' . __('Import Process Complete','profilegrid-user-profiles-groups-and-communities') . '</strong><br />';
    echo '<i class="fa fa-user-plus" aria-hidden="true"></i> ' . __('New Users Created','profilegrid-user-profiles-groups-and-communities').' - '.$created.'<br/>' ;
    echo '<i class="fa fa-user" aria-hidden="true"></i> ' . __('Existing Users Updated','profilegrid-user-profiles-groups-and-communities').' - '.$updated.'<br/>' ;
    echo '<i class="fa fa-user-times" aria-hidden="true"></i> ' . __('Total Users Skipped','profilegrid-user-profiles-groups-and-communities').' - '.$skipped.'<br/></div>' ;
     echo '<b>' . __('Import Details','profilegrid-user-profiles-groups-and-communities') . '</b><br/>';
  ...
  public function pm_import_user($data,$post,$headings)
  {
    ...
    else
    {
        ...
        if(is_numeric($user_id)):
            ...
            $log['status'] = 0;
            $log['message'] = sprintf(__('User not created with following username <b>%s</b> and email <b>%s<b>','profilegrid-user-profiles-groups-and-communities'),$user_name,$user_email);
            return $log;
        endif;
    }
    ...
    {
            wp_update_user( array( 'ID' => $user_id, $field_key => $data[$i] ) );
    }
