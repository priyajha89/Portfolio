 ...
 */
	public function validate_ipn() {
		...
		if (isset($this->paypal_mail) && strtolower ( sanitize_email($_POST['receiver_email']) ) != strtolower(trim( $this->paypal_mail ))) {
			$this->ipn_status =  esc_html__("Receiver Email Not Match","profilegrid-user-profiles-groups-and-communities");
			$this->log_ipn_results ( false );
			return false;
		}
		
		if (isset($this->txn_id)&& in_array(sanitize_text_field($_POST['txn_id']),$this->txn_id)) {
