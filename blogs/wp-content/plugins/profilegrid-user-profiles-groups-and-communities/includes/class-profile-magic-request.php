	...
	public function profile_magic_get_error_message( $error_code, $textdomain ) {
		switch ( $error_code ) {
			...
			case 'incorrect_password':
				$err = wp_kses_post( "You entered incorrect password. Please try again or click on <a href='%s'>“Forgot Password”</a>", 'profilegrid-user-profiles-groups-and-communities' );
			...
			case 'payment_pending':
				...
				$message = sprintf( $url, $this->pm_get_repayment_url(sanitize_text_field($_REQUEST['id'] )) );
				break;
			...
			case 'loginrequired':
								$err            = wp_kses_post( "Login required to view this page. Please <a href='%s'>Login</a>.", 'profilegrid-user-profiles-groups-and-communities' );
	...
	<tbody>
		...
		<!--********************************-->
			...
			<?php foreach ( $posts_array as $post ) : ?>
				...
				?>
		<tr>
			<td><input class="pg-blog-checked  
				<?php
 ...
 <table class="pg-group-members">
	<tbody>
		...
		<tr>
			...
			<td><input class="pg-member-checked 
				<?php
				if ( $user_status == '1' ) {
					echo 'inactive';
				} else {
	 ...
	 <div class='pg-alert-warning pg-alert-info'><?php esc_html_e( 'There are no members in this group. You can start inviting new members by clicking <i>Add</i> Button.', 'profilegrid-user-profiles-groups-and-communities' ); ?></div>
			<?php
		}
...
<input type="hidden" name="pg_total_group_members_sigle_page_grid" id="pg_total_group_members_sigle_page_grid" value="<?php echo esc_attr( $limit ); ?>" />
	...
	public function profile_magic_get_join_group_button( $gid, $show_action = 'no' ) {
		...
		$registration_url = add_query_arg( 'gid', $gid, $registration_url );
		if ( $show_action == 'yes' ) {
			$group_page = $this->profile_magic_get_frontend_url( 'pm_group_page', '' );
		...
		if ( is_user_logged_in() ) {
							...
							<div class="pm-group-signup">
								<form method="post" <?php echo wp_kses_post( $action ); ?>>
									...
									<a class="pm_button" onclick="submit()">
										<button type="submit"><?php esc_html_e( 'Join Group', 'profilegrid-user-profiles-groups-and-communities' ); ?></button>
									</a>
								</form>
							</div>
							<?php
			...
			if ( $is_group_limit == 1 ) {
				if ( $limit > $total_users_in_group ) {
					?>
				<div class="pm-group-signup">
					<a href="<?php echo esc_url( $registration_url ); ?>" class="pm_button">
						<button><?php esc_html_e( 'Join Group', 'profilegrid-user-profiles-groups-and-communities' ); ?></button>
					</a>
				</div>
					<?php
				} else {
...
<table class="pg-group-requests pg-group-members">
	...
	public function pg_get_group_card_icon_link( $gid ) {
		...
		if ( $group_page == '0' ) {
			$html = '<a onclick="pg_create_group_page(' . $gid . ')" title="' . esc_attr__( 'Click to create page for this group', 'profilegrid-user-profiles-groups-and-communities' ) . '"><i class="fa fa-plus"></i></a>';
		} else {
		...
		if ( $dbhandler->get_global_option_value( $option, '1' ) == '1' ) {
			if ( $view == 'grid' ) {
				...
				<div class="pm-group pm-difl pm-border pm-radius5 pm-bg-lt pm50">
				  <div class="pm-group-heading pm-dbfl pm-border-bt pm-pad10 pm-clip"><a href="<?php echo esc_url( $group_url ); ?>"><?php echo esc_html( $group->group_name ); ?></a></div>
				  ...
				  <div class="pm-group-button pm-dbfl pm-pad10">
				   <div class="pm-group-signup"><a href="<?php echo esc_url( $registration_url ); ?>" class="pm_button"><button><?php esc_html_e( 'Join Group', 'profilegrid-user-profiles-groups-and-communities' ); ?></button></a></div>
				  ...
				  <div class="pm-group-button pm-dbfl pm-pad10">
				   <div class="pm-group-signup">
									   <a href="<?php echo esc_url( $group_url ); ?>" class="pm_button">
						   <button><?php esc_html_e( 'Details', 'profilegrid-user-profiles-groups-and-communities' ); ?></button>
					   </a>
				   </div>
				  </div>
				  <?php endif; ?>
		 ...
		 <div class="pm-group-list-view pm-dbfl">
			 ...
			 <div class="pm-group-name-desc pm-difl">
				 <div class="pm-group-heading pm-dbfl pm-pad10 pm-clip"><a href="<?php echo esc_url( $group_url ); ?>"><?php echo esc_html( $group->group_name ); ?></a></div>
				 <?php
									$groupdesc = '';
					if ( strlen( $group->group_desc ) > 150 ) {
							$groupdesc  = substr( $group->group_desc, 0, 150 );
