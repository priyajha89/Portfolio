...
if(isset($post['edit_profile']))
{
	...
	$gids = $pmrequests->profile_magic_get_user_field_value($post['euid'],'pm_group');
        ...
        if(empty($errors))
	{
            $pmrequests->pm_update_user_custom_fields_data($_POST,$_FILES,$_SERVER,$gid,$fields,$post['euid']);
            do_action('profile_magic_update_user_meta',$_POST,$_FILES,$_SERVER,$gid,$fields,$post['euid'],$textdomain);
	}
...
if(isset($uid))
{
	...
	$gids = maybe_unserialize($pmrequests->profile_magic_get_user_field_value($uid,'pm_group'));
        ...
        echo '<div class="pm_message">'. esc_html__('This user does not exists.','profilegrid-user-profiles-groups-and-communities').'</div>';
    endif;   
