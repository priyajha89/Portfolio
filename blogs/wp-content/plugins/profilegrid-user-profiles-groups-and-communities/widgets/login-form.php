...
{
    $retrieved_nonce = $request['_wpnonce'];
    if (!wp_verify_nonce($retrieved_nonce, 'pm_login_form' ) ) die( esc_html__('Failed security check','profilegrid-user-profiles-groups-and-communities') );
...
if ( is_user_logged_in()) : 
 ...
 <div class="pg-user-content pm-dbfl pm-border">
     ...
     if (is_user_logged_in())
       ...
       ?>
     <div class="pg_widget_logout_link pm-dbfl pm-border pm-pad10"><a href="<?php echo esc_url($login_page_url_widget); ?>">Logout</a></div>
      ...
      <div class="pm_widget-login-row pm-difl">
        ...
        <input type="password" name="<?php echo esc_attr('user_pass');?>" id="<?php echo esc_attr('user_pass');?>" placeholder="<?php esc_attr_e('Password','profilegrid-user-profiles-groups-and-communities');?>" required="required">
            ...
            <div class="pm-login-box-bottom-container pm-dbfl">
                <input type="submit" value="<?php esc_attr_e('Login','profilegrid-user-profiles-groups-and-communities');?>" name="login_form_submit" class="">
